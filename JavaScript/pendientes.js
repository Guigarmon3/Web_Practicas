...
document.addEventListener('DOMContentLoaded', async () => {
    await cargarTodosLosPagosPendientes();
...
function renderizarPagos(lista) {
    const mainContainer = document.getElementById('contenedor-pendientes');
    mainContainer.innerHTML = '';

    if (lista.length === 0) {
        mainContainer.innerHTML = `<p style="text-align:center; font-weight:bold; padding:20px;">Sin pagos pendientes.</p>`;
        return;
    }
